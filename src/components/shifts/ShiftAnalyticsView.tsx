...
export const ShiftAnalyticsView: React.FC<ShiftAnalyticsViewProps> = ({ sessions }) => {
  const [timeframe, setTimeframe] = useState<'all' | 'thisMonth' | '30d' | '7d'>('all');

  // Filter sessions by timeframe
  const filteredSessions = useMemo(() => {
    const today = getJakartaDate();
    const [currY, currM] = today.split('-').map(Number);
    const monthPrefix = `${currY}-${String(currM).padStart(2, '0')}`;
    ...
    if (timeframe === '7d' || timeframe === '30d') {
      ...
      return sessions.filter((s) => s.businessDate >= cutoff);
    }
  ...
  const shiftAggregates = useMemo(() => {
    ...
    return shiftDefs.map((def) => {
      const matched = filteredSessions.filter(
  ...
  // Identify Best Shift by Revenue, Conversion, and Viewers
  const bestRevenueShift = useMemo(() => {
    return [...shiftAggregates].sort((a, b) => b.revenue - a.revenue)[0];
  }, [shiftAggregates]);

  const bestCrShift = useMemo(() => {
    return [...shiftAggregates].sort((a, b) => b.conversionRate - a.conversionRate)[0];
  }, [shiftAggregates]);

  const bestTrafficShift = useMemo(() => {
    return [...shiftAggregates].sort((a, b) => b.viewers - a.viewers)[0];
  }, [shiftAggregates]);

  // Export
  const handleExportShifts = () => {
    const headers = ['Shift', 'Jam Operasional', 'Jumlah Sesi', 'Total Omset (IDR)', 'Orders', 'Viewers', 'Conversion (%)', 'Revenue / Jam (IDR)'];
    const rows = shiftAggregates.map((sh) => [
      sh.name,
      sh.time,
      sh.sessionsCount,
      sh.revenue,
    ...
    exportToExcel({
      title: 'Laporan Analisis Komparasi Shift Live Streaming',
      periodDescription: timeframe === 'thisMonth' ? 'Bulan Ini' : timeframe === '7d' ? '7 Hari Terakhir' : timeframe === '30d' ? '30 Hari Terakhir' : 'Semua Periode',
      summaryKpis: [
        { label: 'Total Sesi Dianalisis', value: `${filteredSessions.length} Sesi` },
        { label: 'Shift Paling Menghasilkan', value: bestRevenueShift ? bestRevenueShift.name : '-' },
        { label: 'Shift Konversi Tertinggi', value: bestCrShift ? `${bestCrShift.name} (${bestCrShift.conversionRate}%)` : '-' },
      ],
      headers,
      rows,
      fileNamePrefix: `Shift_Analytics_${timeframe}`,
    });
  };
    ...
    <div className="space-y-6">
          ...
          <div className="flex flex-wrap items-center gap-2.5">
            ...
            <div className="flex items-center bg-slate-100 p-0.5 rounded-xl border border-slate-200 text-xs">
              ...
              <button
                ...
                className={`px-3 py-1 rounded-lg font-bold transition-colors cursor-pointer ${
                  timeframe === 'thisMonth' ? 'bg-white text-slate-800 shadow-xs' : 'text-slate-500 hover:text-slate-800'
                }`}
              >
                Bulan Ini
              </button>
            ...
            <button
              ...
              className="px-3.5 py-1.5 text-xs font-bold text-slate-700 bg-white hover:bg-slate-50 border border-slate-200 rounded-xl shadow-xs transition-all active:scale-95 flex items-center gap-1.5 cursor-pointer"
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        {shiftAggregates.map((sh) => {
          const Icon = sh.icon;
          const isBestRev = bestRevenueShift && bestRevenueShift.id === sh.id && sh.revenue > 0;
          const isBestCr = bestCrShift && bestCrShift.id === sh.id && sh.conversionRate > 0;

          return (
            <div
              key={sh.id}
              className={`clay-card p-6 space-y-4 relative ${
                isBestRev ? 'border-orange-300 ring-2 ring-orange-200 bg-gradient-to-b from-orange-50/50 to-white' : ''
              }`}
            >
              <div className="flex items-center justify-between">
                ...
                <div className="flex flex-col items-end gap-1">
                  {isBestRev && (
                    <span className="px-2.5 py-0.5 text-[9px] font-black uppercase tracking-wider bg-orange-600 text-white rounded-lg shadow-xs flex items-center gap-1">
                      <Flame className="w-3 h-3" />
                      TOP OMSET
                    </span>
                  )}
                  {isBestCr && (
                    <span className="px-2.5 py-0.5 text-[9px] font-black uppercase tracking-wider bg-emerald-600 text-white rounded-lg shadow-xs flex items-center gap-1">
